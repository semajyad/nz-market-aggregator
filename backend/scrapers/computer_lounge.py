...
class ComputerLoungeScraper(BaseScraper):
    ...
    async def search(self, query: ParsedQuery) -> List[NormalizedItem]:
        ...
        url = f"{self.base_url}/components/search.asp?search={encoded}"
        ...
        self.logger.info(f"Computer Lounge found {len(items)} raw items")
        ...
        self.logger.info(f"Computer Lounge returning {len(results)} results")
        return results
