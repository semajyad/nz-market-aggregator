...
class HarveyNormanScraper(BaseScraper):
    platform_name = Platform.HARVEY_NORMAN
    ...
    async def search(self, query: ParsedQuery) -> List[NormalizedItem]:
        ...
        for search_term in search_terms:
            ...
            for url in candidate_urls:
                self.logger.info(f"Harvey Norman searching: {url}")
                ...
                self.logger.info(f"Harvey Norman found {len(items)} raw items for term '{search_term}'")
                ...
                for item in items[:80]:
                    try:
                        ...
                        self.logger.debug(f"Error parsing Harvey Norman item: {e}")
                        continue
                ...
                for form in soup.select("form[name^='product_form_']")[:80]:
                    link_el = form.select_one("a.product-title, a[href*='dispatch=products.view'], a[href*='/product/']")
                    if not link_el:
                        continue
                    item_url = self._normalize_listing_url(link_el.get("href", ""))
                    ...
                    price_el = form.select_one(".ty-price-num, .price, [class*='price']")
                    price_text = price_el.get_text(strip=True) if price_el else ""
                    price_val, price_display = self._clean_price(price_text)
                    if query.max_price and price_val and price_val > query.max_price:
                        continue

                    img_el = form.select_one("img[src], img[data-src], img[data-srcset]")
                    image_url = None
                    if img_el:
            ...
            fallback_links = self._fallback_links_from_duckduckgo("harveynorman.co.nz", search_term, max_links=20)
            for link in fallback_links:
                results.append(NormalizedItem(
                    title=self._truncate(f"Harvey Norman - {search_term}", 150),
                    price=None,
        ...
        self.logger.info(f"Harvey Norman returning {len(deduped)} results")
        return deduped[:100]
