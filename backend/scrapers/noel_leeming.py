...
class NoelLeemingScraper(BaseScraper):
    platform_name = Platform.NOEL_LEEMING
    ...
    async def search(self, query: ParsedQuery) -> List[NormalizedItem]:
        ...
        for search_term in search_terms:
            ...
            for url in candidate_urls:
                self.logger.info(f"Noel Leeming searching: {url}")
                html = await self._get_page_html(url, extra_wait_ms=2000)
                if not html:
                    continue

                soup = BeautifulSoup(html, "lxml")
                items = (
                    soup.select("[data-testid='product-tile']")
                ...
                self.logger.info(f"Noel Leeming found {len(items)} raw items for term '{search_term}'")
                ...
                for item in items[:70]:
                    try:
                        title_el = (
                            ...
                            or item.select_one("a.link.text-emphasized")
                            or item.select_one(".product-tile-details a[href]")
                        ...
                        self.logger.debug(f"Error parsing Noel Leeming item: {e}")
                        continue

            fallback_links = self._fallback_links_from_duckduckgo("noelleeming.co.nz", search_term, max_links=20)
            for link in fallback_links:
                results.append(NormalizedItem(
                    title=self._truncate(f"Noel Leeming - {search_term}", 150),
                    price=None,
        ...
        self.logger.info(f"Noel Leeming returning {len(deduped)} results")
        return deduped[:80]
